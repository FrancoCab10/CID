...
- `cid.src`/`uuid.src` at the repo root are the source of truth, written
  using greybel syntax extensions (like `/* */` comments) that only the
  greybel language server/CLI understand — vanilla GreyScript, and Grey
  Hack's in-game code editor, don't. `dist/cid.src`/`dist/uuid.src` are the
  plain-GreyScript equivalent, auto-generated by
  `.github/workflows/build-ingame-src.yml` (via `greybel build --beautify`)
  on every push to `main`, and committed back by that workflow — never hand
...
- Document public functions with a `/* */` block comment directly above the
  function. `/* */` is a greybel-only extension — vanilla GreyScript only has
  `//` — so `/*` and `*/` must never sit alone on their own line: the build
  pipeline (see Architecture) converts every physical line to a `//` line
  as-is, so a lone `/*` or `*/` line becomes a stray bare `//` in the
  generated in-game source. Always attach the delimiters to the first and
